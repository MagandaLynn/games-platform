...
function rankPlay(play: RawPlay) {
  ...
  const attemptsWeight = play.attempts === null ? 0 : Math.max(0, 100 - play.attempts);
  return solvedWeight + attemptsWeight;
}
...
async function queryPlays(profile: ProfileRef, from: Date, to: Date) {
  ...
  try {
    ...
    if (message.includes("WordleDailyPlay") && (message.includes("does not exist") || message.includes("42P01"))) {
      return [];
    }
    throw error;
  }
...
async function collectForProfile(profile: ProfileRef, axisDates: string[], from: Date, to: Date) {
  ...
  const daily: DailyPoint[] = axisDates.map((date) => {
    ...
    const guessedCount = play
      ? play.attempts ?? ((play.maxGuesses ?? 6) + 1)
      : 0;
    const perfect = Boolean(play?.solved && play?.attempts === 1);

    return {
      date,
      importedOn: play ? toDateKey(play.importedAt) : null,
      wordleRawText: play?.rawText ?? null,
      wordlePuzzleNumber: play?.puzzleNumber ?? null,
      wordleAttempts: play?.attempts ?? null,
      wordleMaxGuesses: play?.maxGuesses ?? null,
      attempted,
      completed,
      won,
      lost,
...
async function collectLifetimeSummaryForProfile(profile: ProfileRef, to: Date) {
  ...
  const daily: DailyPoint[] = Array.from(bestPlayByPuzzle.values()).map((play) => {
    ...
    const guessedCount = play.attempts ?? ((play.maxGuesses ?? 6) + 1);
    const perfect = Boolean(play.solved && play.attempts === 1);

    return {
      date: String(play.puzzleNumber),
      importedOn: toDateKey(play.importedAt),
      wordleRawText: play.rawText,
      wordlePuzzleNumber: play.puzzleNumber,
      wordleAttempts: play.attempts,
      wordleMaxGuesses: play.maxGuesses,
      attempted,
      completed,
      won,
      lost,
...
export async function GET(req: Request) {
  try {
    ...
    return Response.json(
      { error: "COMPARE_READ_FAILED", message },
      { status: 500 }
    );
  }
}
